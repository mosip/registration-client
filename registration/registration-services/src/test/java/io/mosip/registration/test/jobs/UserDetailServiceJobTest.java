...
//				
	...
	@Test(expected = RegBaseUncheckedException.class)
	public void executeJobTest() throws JobExecutionException {
		SyncJobDef syncJob = new SyncJobDef();
		syncJob.setId("1");
		...
		userDetailServcieJob.executeParentJob("1");
...
//		Mockito.when(jobManager.getChildJobs(Mockito.any())).thenReturn(jobMap);
		...
		Mockito.when(context.getJobDetail()).thenThrow(RuntimeException.class);
