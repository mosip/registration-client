...
@PrepareForTest({ HMACUtils2.class, SessionContext.class, FileUtils.class, JsonUtils.class, BigInteger.class, CryptoUtil.class })
public class PacketSynchServiceImplTest {
	...
	//@Mock
	//private RegistrationDAOImpl syncRegistrationDAO;
	...
	@Mock
	private OfflinePacketCryptoServiceImpl offlinePacketCryptoServiceImpl;
	...
	public void initialize() throws Exception{
		...
		maplastTime.put(RegistrationConstants.USER_CENTER_ID, "10008");
		maplastTime.put(RegistrationConstants.USER_STATION_ID, "10008");
	...
	public void testFetchPacketsToBeSynched() {
		...
		Mockito.when(registrationDAOImpl.fetchPacketsToUpload(Mockito.anyList(),Mockito.anyString())).thenReturn(syncList);
	...
	public void packetSyncTest() throws RegBaseCheckedException, ConnectionException, NoSuchAlgorithmException {
		...
		Mockito.when(serviceDelegateUtil.post(Mockito.anyString(), Mockito.anyString(), Mockito.anyString()))
				.thenReturn(obj);
		Mockito.when(registrationDAOImpl.updatePacketSyncStatus(packetStatusDTO)).thenReturn(reg);
	...
	public void syncPacketPassWithPassedPacketIdsTest() throws Exception {
		...
		expectedError.setCode("ERROR");
		expectedError.setInfoType("ERROR");
		expectedError.setMessage("Packet sync");
		Map<String, Object> mapOfAttributes = new HashMap<String, Object>();
		expectedError.setOtherAttributes(mapOfAttributes);
		...
		Mockito.when(registrationDAOImpl.get(Mockito.anyList())).thenReturn(listOfReg);
		Mockito.when(registrationDAOImpl.getPacketsToBeSynched(RegistrationConstants.CLIENT_STATUS_TO_BE_SYNCED, 10)).thenReturn(listOfReg);
		
		ResponseDTO actualResponse = packetSynchServiceImpl.syncPacket("System");
		
		assertEquals(expectedError.getCode(), actualResponse.getErrorResponseDTOs().get(0).getCode());
	}
	...
	public void syncPacketPassWithDBPacketIdsTest() throws Exception {
		...
		expectedSuccess.setCode("INFORMATION");
		...
		Mockito.when(serviceDelegateUtil.isNetworkAvailable()).thenReturn(true);
		Mockito.when(SessionContext.isSessionContextAvailable()).thenReturn(false);
		doReturn(listOfReg).when(registrationDAOImpl).get(Mockito.anyList());
		doReturn(listOfReg).when(registrationDAOImpl).getPacketsToBeSynched(RegistrationConstants.CLIENT_STATUS_TO_BE_SYNCED, 10);
	...
	@Test
	public void syncPacketCoonectionFailedException() throws Exception {
	...
	@Test
	public void syncPacketPassWithDBPacketIdsConnectionFailed() throws Exception {
	...
	/*@Test
	public void syncRIDToServerTestPositive() throws Exception {
		...
		String encodedStr = "kasjhjahsdhjdhas23978723saldsa";
		String JSONStr = "{\"name\":\"john\", \"age\":22, \"class\":\"mca\"}";
		List<Registration> listOfReg = new ArrayList<Registration>();
		
		Registration reg1 = new Registration();
		reg1.setId("101202");
		reg1.setClientStatusCode("");
		reg1.setStatusCode("PACKET SYNC");
		reg1.setAckFilename("djaldakj_Ack.html");
		byte[] additionalInfoByte = "kasjhjahsdhjdhas23978723saldsa".getBytes();
		reg1.setAdditionalInfo(additionalInfoByte);
		reg1.setPacketId("101202");
		listOfReg.add(reg1);
		
		RegistrationDataDto registrationDataDto = new RegistrationDataDto();
		registrationDataDto.setEmail("mosip@gmail.com");
		registrationDataDto.setLangCode("eng");
		registrationDataDto.setName("MOSIP");
		registrationDataDto.setPhone("7328638276");
		
		LinkedHashMap<String, Object> response = new LinkedHashMap<>();
		
		List<LinkedHashMap<String, Object>> listResponse = new ArrayList<>();
		LinkedHashMap<String, Object> list1 = new LinkedHashMap<>();
		list1.put("registrationId", "ksadhhd");
		list1.put("status", "Success");
		listResponse.add(list1);
		
		response.put("response", listResponse);
		
		ResponseDTO responseTemp = new ResponseDTO();
		...
		PowerMockito.mockStatic(CryptoUtil.class, JsonUtils.class);
		Mockito.when(JsonUtils.javaObjectToJsonString(Mockito.any())).thenReturn(encodedStr);
		Mockito.when(JsonUtils.jsonStringToJavaObject(Mockito.any(), Mockito.anyString())).thenReturn(registrationDataDto);
		Mockito.when(CryptoUtil.encodeToURLSafeBase64(additionalInfoByte)).thenReturn(encodedStr);
		Mockito.when(serviceDelegateUtil.post(Mockito.anyString(), Mockito.anyString(), Mockito.anyString())).thenReturn(response);
		PowerMockito.when(packetSynchServiceImpl, "syncPacketsToServer", JSONStr, "Jeevan", true).thenReturn(responseTemp);
		Mockito.when(registrationDAOImpl.updatePacketSyncStatus(Mockito.anyObject())).thenReturn(reg1);
		doReturn(listOfReg).when(registrationDAOImpl).get(Mockito.anyList());
		doReturn(listOfReg).when(registrationDAOImpl).getPacketsToBeSynched(RegistrationConstants.CLIENT_STATUS_TO_BE_SYNCED, 10);
