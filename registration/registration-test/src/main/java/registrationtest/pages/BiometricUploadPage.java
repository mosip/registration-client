		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				try {
					 ...
					 * 
					 * Optional<Node> opNode = hbox.getChildren().stream() .filter(node -> (node
					 * instanceof Button) && ((Button)
					 * node).getId().equalsIgnoreCase(idModality.replace("#", "") + "Button"))
					 * .findFirst(); if (opNode.isPresent()) { ((Button) opNode.get()).fire(); }
					 */
...
// ================= IRIS =================
			if (hasIris(list)) {
				...
				if (hasIrisException(listException)) {
...
// ================= RIGHT HAND =================
			if (hasRightFingers(list)) {

				if (hasRightFingerException(listException)) {
...
// ================= LEFT HAND =================
			if (hasLeftFingers(list)) {

				if (hasLeftFingerException(listException)) {
...
// ================= THUMBS =================
			if (hasThumbs(list)) {
				...
				if (hasThumbException(listException)) {
...
// ================= EXCEPTION PHOTO =================
			 if (!id.equals("#") && !listException.isEmpty()) {
		            bioScan(id, id + EXCEPTION_PHOTO, identity);
		      }

		} catch (Exception e) {
			logger.error("Bio upload failed", e);
		}
	}

	private boolean hasIrisException(List<String> ex) {
		return ex.contains("leftEye") || ex.contains("rightEye");
	}

	private boolean hasRightFingerException(List<String> ex) {
		return ex.contains("rightIndex") || ex.contains("rightLittle") || ex.contains("rightRing")
				|| ex.contains("rightMiddle");
	}

	private boolean hasLeftFingerException(List<String> ex) {
		return ex.contains("leftIndex") || ex.contains("leftLittle") || ex.contains("leftRing")
				|| ex.contains("leftMiddle");
	}

	private boolean hasThumbException(List<String> ex) {
		return ex.contains("leftThumb") || ex.contains("rightThumb");
	}
	
	private boolean hasIris(List<String> list) throws IOException{
	    return list.contains(PropertiesUtil.getKeyValue("leftEye")) ||
	           list.contains(PropertiesUtil.getKeyValue("rightEye"));
	}
	
	private boolean hasRightFingers(List<String> list) throws IOException{
	    return list.contains(PropertiesUtil.getKeyValue("rightIndex")) ||
	...
	private boolean hasLeftFingers(List<String> list) throws IOException{
	    return list.contains(PropertiesUtil.getKeyValue("leftIndex")) ||
	...
	private boolean hasThumbs(List<String> list) throws IOException{
	    return list.contains(PropertiesUtil.getKeyValue("leftThumb")) ||
