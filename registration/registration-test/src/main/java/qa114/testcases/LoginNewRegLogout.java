...
public class LoginNewRegLogout {
	...
	public boolean initialRegclientSet(FxRobot robot,String loginUserid,String loginPwd,Stage applicationPrimaryStage1)
	{
		...
		try {
			...
			if(flag==true)
			{
				ExtentReportUtil.test.log(Status.PASS, "SUCCESS Restart Application");
			}
			else
			{
	...
	public RID newRegistrationAdult(FxRobot robot,String loginUserid,String loginPwd,String supervisorUserid,
		...
		try {
		ExtentReportUtil.ExtentSetting();
		ExtentReportUtil.test=ExtentReportUtil.reports.createTest("New Adult Registration Scenario");
		...
		ExtentReportUtil.step4.log(Status.PASS, "Accept Preview done");
		...
		ExtentReportUtil.step5.log(Status.PASS, "Approve Packet done");
