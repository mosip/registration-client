...
@Controller
public class BiometricsScanPopUpController extends BaseController {
	private static final Logger LOGGER = AppConfig.getLogger(BiometricsScanPopUpController.class);
	...
	public void init(BaseController parentControllerObj, String title) {
		try {
			...
			LOGGER.info(LOG_REG_IRIS_CAPTURE_CONTROLLER, APPLICATION_NAME, APPLICATION_ID, "loading BIOMETRICS_SCAN_PAGE");
			Parent scanPopup = BaseController.load(getClass().getResource(RegistrationConstants.BIOMETRICS_SCAN_PAGE));
