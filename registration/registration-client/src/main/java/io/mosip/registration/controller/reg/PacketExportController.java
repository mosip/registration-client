			...
			if (destinationPath != null) {
				...
				for (PacketStatusDTO packetToCopy : packetsToBeExported) {
					...
					if (packet.length() < destinationPath.getUsableSpace()) {
						...
						generateAlert(RegistrationConstants.ERROR, RegistrationUIConstants.getMessageLanguageSpecific("PACKET_EXPORT_FAILURE"));
						break;
					}
				}
			}
		} else {
			generateAlert(RegistrationConstants.ALERT_INFORMATION, RegistrationUIConstants.getMessageLanguageSpecific("PACKET_EXPORT_MESSAGE"));
		}
